...
def ensure_root_admin(sender, **kwargs):
    """
    ضمان وجود المستخدم الجذر ammar دائماً بعد كل migrate:
    - إنشاؤه إذا لم يكن موجوداً
    - تحديث كلمة المرور والصلاحيات إذا تغيّرت
    - إنشاء UserProfile مرتبط به إذا لم يكن موجوداً
    """
    try:
        ...
        # تأكد دائماً من صحة الصلاحيات وكلمة المرور
        needs_save = False
        if not user.check_password(ROOT_ADMIN_PASSWORD):
