...
from .models import Product, Location, Warehouse, AuditLog, Order, ProductReturn, UserProfile, UserActivityLog, Container, SecureBackup, Invoice
...
def convert_to_hijri(gregorian_date):
    """تحويل التاريخ الميلادي إلى هجري بدقة باستخدام مكتبة hijridate"""
    _HIJRI_MONTHS_AR = [
    ...
    try:
        import hijridate
        h = hijridate.Gregorian(
            gregorian_date.year, gregorian_date.month, gregorian_date.day
        ).to_hijri()
        month_ar = _HIJRI_MONTHS_AR[h.month - 1]
        return f"{h.day} {month_ar} {h.year} هـ"
    except Exception:
...
def get_all_recipients_stats(request):
    """API لجلب إحصائيات جميع المستلمين (يدوي + فواتير)"""
    from django.db.models import Count, Sum
    combined = {}

    # الطلبات اليدوية
    for r in (
        Order.objects.exclude(recipient_name__isnull=True).exclude(recipient_name='')
        ...
        combined[name] = {
            'recipient_name': name,
            'count': combined.get(name, {}).get('count', 0) + r['count'],
            'total_qty': combined.get(name, {}).get('total_qty', 0) + (r['total_qty'] or 0),
        }

    # عملاء الفواتير
    for inv in Invoice.objects.exclude(customer_name='').values('customer_name').annotate(count=Count('id')):
        name = inv['customer_name']
        if name in combined:
            combined[name]['count'] += inv['count']
        else:
            combined[name] = {'recipient_name': name, 'count': inv['count'], 'total_qty': 0}

    result = sorted(combined.values(), key=lambda x: x['count'], reverse=True)
    return JsonResponse({'success': True, 'recipients': result})
...
def orders_list(request):
    ...
    # 1. إحصائيات الرسم البياني: حركة السحب اليومية (آخر 30 يوم) — يدوي + فواتير
    ...
    daily_stats_inv = (
        Invoice.objects.filter(created_at__gte=last_30_days)
        .annotate(date=TruncDate('created_at'))
        .values('date')
    ...
    daily_combined = {}
    for s in daily_stats_orders:
        d = s['date'].strftime('%Y-%m-%d')
        daily_combined[d] = {'count': s['count'], 'qty': s['qty'] or 0}
    for s in daily_stats_inv:
        d = s['date'].strftime('%Y-%m-%d')
        if d in daily_combined:
            daily_combined[d]['count'] += s['count']
        else:
            daily_combined[d] = {'count': s['count'], 'qty': 0}

    sorted_daily = sorted(daily_combined.items())
    daily_labels = [d for d, _ in sorted_daily]
    daily_counts  = [v['count'] for _, v in sorted_daily]
    daily_qtys    = [v['qty']   for _, v in sorted_daily]

    # 2. أكثر المستلمين نشاطاً (Top 5) — يدوي + فواتير
    rec_combined = {}
    for r in (
        Order.objects.exclude(recipient_name__isnull=True).exclude(recipient_name='')
        .values('recipient_name').annotate(count=Count('id'))
    ):
        rec_combined[r['recipient_name']] = rec_combined.get(r['recipient_name'], 0) + r['count']
    for inv in (
        Invoice.objects.exclude(customer_name='')
        .values('customer_name').annotate(count=Count('id'))
    ):
        n = inv['customer_name']
        rec_combined[n] = rec_combined.get(n, 0) + inv['count']

    top5 = sorted(rec_combined.items(), key=lambda x: x[1], reverse=True)[:5]
    recipient_labels = [k for k, _ in top5]
    recipient_counts = [v for _, v in top5]
    ...
    # حساب السعر الإجمالي لكل طلب في الصفحة الحالية
    page_order_ids = [o.id for o in page_obj]
    page_product_numbers = set()
    page_orders_products = {}
    for order in page_obj:
        ...
        if isinstance(prods, str):
            try:
                ...
                prods = []
        page_orders_products[order.id] = prods if isinstance(prods, list) else []
        for p in page_orders_products[order.id]:
            num = p.get('product_number', '')
            if num:
                page_product_numbers.add(num)

    page_prices = {
        p.product_number: float(p.price) if p.price else 0
        for p in Product.objects.filter(product_number__in=page_product_numbers)
    }
    orders_totals = {}
    for order in page_obj:
        total = 0.0
        for p in page_orders_products.get(order.id, []):
            num = p.get('product_number', '')
            qty = int(p.get('quantity_taken', p.get('quantity', 0)))
            p12 = page_prices.get(num, 0)
            total += (p12 / 12) * qty if p12 else 0
        order_total = round(total, 2)
        orders_totals[order.id] = order_total
        order.order_total = order_total   # أضف مباشرة على الكائن

    # ── الفواتير ──
    invoices_qs = Invoice.objects.all().order_by('-created_at')
    inv_page_number = request.GET.get('inv_page', 1)
    inv_paginator = Paginator(invoices_qs, 20)
    inv_page_obj = inv_paginator.get_page(inv_page_number)
    total_invoices = invoices_qs.count()
    today_invoices = invoices_qs.filter(created_at__date=timezone.now().date()).count()
    ...
    context = {
        ...
        # الفواتير
        'invoices': inv_page_obj,
        'inv_page_obj': inv_page_obj,
        'total_invoices': total_invoices,
        'today_invoices': today_invoices,
        # أسعار الطلبات في الصفحة الحالية
...
def search_order_history(request):
    """بحث ذكي في سجلات الطلبات اليدوية والفواتير معاً"""
    query = request.GET.get('q', '').strip()
    if not query:
    ...
    # ── 1. الطلبات اليدوية ──
    for order in Order.objects.filter(products_data__icontains=query):
        try:
            products = order.products_data
            if isinstance(products, str):
                products = json.loads(products)
    ...
    # ── 2. فواتير عرض الأسعار ──
    for inv in Invoice.objects.filter(products_data__icontains=query):
        try:
            products = inv.products_data
            if isinstance(products, str):
                products = json.loads(products)
            for p in products:
                p_name = str(p.get('name', '')).lower()
                p_num  = str(p.get('number', '')).strip().lower()
                if q_lower == p_num or q_lower in p_name:
                    qty = int(p.get('qty', 0))
                    total_withdrawn += qty
                    r_name = inv.customer_name or 'عميل فاتورة'
                    recipients_map[r_name] = recipients_map.get(r_name, 0) + qty
...
def order_detail(request, order_id):
    ...
    for p in order.products_data:
        ...
        if p_num in products_map:
            ...
            price12 = float(product.price) if product.price is not None else 0
            p['price12'] = price12                        # سعر الدرزن
            if product.image:
        ...
        qty = int(p.get('quantity_taken', 0) or 0)
        line_total = (price12 / 12) * qty if price12 else 0
        p['line_total'] = round(line_total, 2)
...
def view_invoice(request, invoice_id):
    """عرض / طباعة فاتورة محفوظة"""
    invoice = get_object_or_404(Invoice, id=invoice_id)
    return render(request, 'inventory_app/invoice_view.html', {'invoice': invoice})


@login_required
def create_invoice(request):
    """صفحة إنشاء فاتورة عرض الأسعار"""
    from django.utils import timezone
    today = timezone.now().date()
    today_str = today.strftime('%Y/%m/%d')
    hijri_str = convert_to_hijri(today)
    next_number = Invoice.next_number()
    return render(request, 'inventory_app/invoice_create.html', {
...
@login_required
@require_http_methods(["GET"])
def api_invoice_product(request):
    """جلب بيانات منتج بواسطة رقمه لاستخدامها في الفاتورة"""
    number = request.GET.get('number', '').strip()
    if not number:
        return JsonResponse({'success': False, 'error': 'رقم المنتج مطلوب'}, json_dumps_params={'ensure_ascii': False})
    try:
        product = Product.objects.get(product_number__iexact=number)
        return JsonResponse({
            'success': True,
            'product': {
        ...
        return JsonResponse({'success': False, 'error': f'المنتج "{number}" غير موجود في النظام'}, json_dumps_params={'ensure_ascii': False})


@login_required
@require_http_methods(["POST"])
@transaction.atomic
def api_save_invoice(request):
    """حفظ الفاتورة مع التحقق من المخزون وخصم الكميات"""
    try:
        data = json.loads(request.body)
        invoice_number = data.get('invoice_number', '').strip()
        if not invoice_number:
            return JsonResponse({'success': False, 'error': 'رقم الفاتورة مطلوب'}, json_dumps_params={'ensure_ascii': False})
        if Invoice.objects.filter(invoice_number=invoice_number).exists():
            return JsonResponse({'success': False, 'error': f'رقم الفاتورة {invoice_number} محفوظ مسبقاً'}, json_dumps_params={'ensure_ascii': False})

        products_data = data.get('products', [])
        if not products_data:
            return JsonResponse({'success': False, 'error': 'يجب إضافة منتج واحد على الأقل'}, json_dumps_params={'ensure_ascii': False})

        # ── جمع أرقام المنتجات وقفلها للتحديث الآمن ──
        product_numbers = [p['number'].strip() for p in products_data if p.get('number', '').strip()]
        if not product_numbers:
            return JsonResponse({'success': False, 'error': 'لا توجد أرقام منتجات صالحة'}, json_dumps_params={'ensure_ascii': False})
        ...
        products_map = {p.product_number.upper(): p for p in locked_products}

        # ── التحقق من وجود كل منتج وكفاية الكمية ──
        errors = []
        for item in products_data:
            num = item.get('number', '').strip().upper()
            if not num:
                continue
            qty = int(item.get('qty', 0))
            if num not in products_map:
                errors.append(f'رقم المنتج "{num}" غير موجود في قاعدة البيانات')
            elif qty > products_map[num].quantity:
                avail = products_map[num].quantity
                errors.append(f'المنتج "{num}": الكمية المطلوبة ({qty}) أكبر من المتوفرة ({avail})')
        ...
        try:
            invoice_date = dt_module.datetime.strptime(date_str, '%Y/%m/%d').date()
        except Exception:
            invoice_date = tz.now().date()

        # ── خصم الكميات وإنشاء سجلات المراجعة ──
        for item in products_data:
            num = item.get('number', '').strip().upper()
            if not num or num not in products_map:
                continue
            qty = int(item.get('qty', 0))
            product = products_map[num]
            ...
            AuditLog.objects.create(
                ...
                quantity_after=product.quantity,
                quantity_change=-qty,
                notes=f'سحب من فاتورة رقم #{invoice_number} - {data.get("customer_name", "")}',
                user=request.user.username,
                product_snapshot={
                    'product_number': product.product_number,
                    'name': product.name,
        ...
        return JsonResponse({
            'success': True,
            'message': f'تم حفظ الفاتورة #{invoice_number} وخصم الكميات من المخزون بنجاح',
...
# ─────────────────────────────────────────────
# API: إجمالي سعر الطلبات اليدوية (price/12 × qty)
# ─────────────────────────────────────────────

@login_required
@require_http_methods(["GET"])
def api_orders_total(request):
    import json as _json
    try:
        ...
        prices_map = {
            p.product_number: float(p.price) if p.price else 0
            for p in Product.objects.filter(product_number__in=all_numbers)
        }
        grand_total = 0.0
        for order in all_orders:
            prods = order.products_data
            ...
            for p in (prods if isinstance(prods, list) else []):
                num = p.get('product_number', '')
                qty = int(p.get('quantity_taken', p.get('quantity', 0)))
                p12 = prices_map.get(num, 0)
                grand_total += (p12 / 12) * qty if p12 else 0
        return JsonResponse(
            {'success': True, 'grand_total': round(grand_total, 2)},
            json_dumps_params={'ensure_ascii': False}
        )
    except Exception as e:
        return JsonResponse(
...
def sales_report(request):
    return render(request, 'inventory_app/sales_report.html')
...
def api_sales_report(request):
    ...
    try:
        ...
        if from_str and to_str:
            try:
                from_dt = timezone.make_aware(
                    dt_module.datetime.strptime(from_str, '%Y-%m-%d')
                )
                from_dt = start_of_day(from_dt)
                to_dt   = timezone.make_aware(
                    dt_module.datetime.strptime(to_str, '%Y-%m-%d')
                )
                to_dt = end_of_day(to_dt)
            except Exception:
                from_dt = today_start
                to_dt   = today_end
        elif period == 'daily':
            # اليوم الحالي من 00:00:00 إلى 23:59:59
            from_dt = today_start
            to_dt   = today_end
        elif period == 'weekly':
            # آخر 7 أيام (اليوم + 6 أيام سابقة)
            from_dt = start_of_day(now - timedelta(days=6))
            to_dt   = today_end
        elif period == 'alltime':
            # كل الوقت
            from_dt = timezone.make_aware(dt_module.datetime(2000, 1, 1))
            to_dt   = today_end
        elif period == 'yearly':
            # آخر 365 يوماً
            from_dt = start_of_day(now - timedelta(days=364))
            to_dt   = today_end
        else:
            # شهري: آخر 30 يوماً
            from_dt = start_of_day(now - timedelta(days=29))
            to_dt   = today_end
        ...
        def ol_total(p):
            ...
            qty = int(p.get('quantity_taken', p.get('quantity', 0)))
            p12 = prices_map.get(num, 0)
            return (p12 / 12) * qty if p12 else 0

        orders_qs   = Order.objects.filter(created_at__gte=from_dt, created_at__lte=to_dt)
        invoices_qs = Invoice.objects.filter(created_at__gte=from_dt, created_at__lte=to_dt)
        returns_qs  = ProductReturn.objects.filter(created_at__gte=from_dt, created_at__lte=to_dt)
        ...
        for order in orders_qs:
            ...
            for p in (prods if isinstance(prods, list) else []):
                ...
                ord_rev      += lt
                if num:
                    if num not in product_stats:
                        product_stats[num] = {'name': name, 'qty': 0, 'revenue': 0.0}
                    product_stats[num]['qty']     += qty
                    product_stats[num]['revenue'] += lt
            cn = order.recipient_name or 'غير محدد'
            if cn not in customer_stats:
                customer_stats[cn] = {'count': 0, 'revenue': 0.0}
            customer_stats[cn]['count']   += 1
            customer_stats[cn]['revenue'] += ord_rev

        invoices_total = float(
            invoices_qs.aggregate(s=db_models.Sum('total'))['s'] or 0
        )
        for inv in invoices_qs:
            prods = inv.products_data if isinstance(inv.products_data, list) else []
            for p in prods:
                num  = p.get('number', '')
                name = p.get('name', '')
                qty  = int(p.get('qty', 0))
                lt   = float(p.get('total', 0))
                if num:
                    if num not in product_stats:
                        product_stats[num] = {'name': name, 'qty': 0, 'revenue': 0.0}
                    product_stats[num]['qty']     += qty
                    product_stats[num]['revenue'] += lt
            cn = inv.customer_name or 'عميل فاتورة'
            if cn not in customer_stats:
                customer_stats[cn] = {'count': 0, 'revenue': 0.0}
            customer_stats[cn]['count']   += 1
            customer_stats[cn]['revenue'] += float(inv.total)

        returns_total = 0.0
        for ret in returns_qs:
            prods = ret.products_data
            if isinstance(prods, str):
                try:
                    prods = _json.loads(prods)
                except Exception:
            ...
            for p in (prods if isinstance(prods, list) else []):
                num = p.get('product_number', '')
                qty = int(p.get('quantity_returned', 0))
                p12 = prices_map.get(num, 0)
                returns_total += (p12 / 12) * qty if p12 else 0

        gross_total = orders_total + invoices_total
        net_total   = max(0.0, gross_total - returns_total)

        day_map = {}
        for order in orders_qs:
            d = order.created_at.strftime('%Y-%m-%d')
            if d not in day_map:
                day_map[d] = {'orders': 0.0, 'invoices': 0.0, 'returns': 0.0}
            prods = order.products_data
            if isinstance(prods, str):
                try:
                    prods = _json.loads(prods)
                    ...
                    prods = []
            for p in (prods if isinstance(prods, list) else []):
                day_map[d]['orders'] += ol_total(p)

        for inv in invoices_qs:
            d = inv.created_at.strftime('%Y-%m-%d')
            if d not in day_map:
                day_map[d] = {'orders': 0.0, 'invoices': 0.0, 'returns': 0.0}
            day_map[d]['invoices'] += float(inv.total)

        for ret in returns_qs:
            d = ret.created_at.strftime('%Y-%m-%d')
            if d not in day_map:
                day_map[d] = {'orders': 0.0, 'invoices': 0.0, 'returns': 0.0}
            prods = ret.products_data
            if isinstance(prods, str):
            ...
            for p in (prods if isinstance(prods, list) else []):
                ...
                qty = int(p.get('quantity_returned', 0))
                p12 = prices_map.get(num, 0)
                day_map[d]['returns'] += (p12 / 12) * qty if p12 else 0

        timeline = [
            {
                'date':     d,
                'orders':   round(v['orders'],   2),
                'invoices': round(v['invoices'], 2),
                'returns':  round(v['returns'],  2),
            }
            for d, v in sorted(day_map.items())
        ]
        top_products  = sorted(product_stats.items(),  key=lambda x: x[1]['revenue'], reverse=True)[:10]
        top_customers = sorted(customer_stats.items(), key=lambda x: x[1]['revenue'], reverse=True)[:10]
        ...
        return JsonResponse({
            ...
            'timeline':      timeline,
            'top_products':  [
                {'number': k, 'name': v['name'], 'qty': v['qty'], 'revenue': round(v['revenue'], 2)}
