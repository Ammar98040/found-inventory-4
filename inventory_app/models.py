...
class Invoice(models.Model):
    ...
    customer_phone = models.CharField(max_length=50, blank=True, verbose_name='رقم الهاتف')
    customer_address = models.TextField(blank=True, verbose_name='العنوان')
    issuer = models.CharField(max_length=200, blank=True, verbose_name='المخرج')
    products_data = models.JSONField(default=list, verbose_name='بيانات المنتجات')
    subtotal = models.DecimalField(max_digits=12, decimal_places=2, default=0, verbose_name='الإجمالي قبل الضريبة')
    ...
    total = models.DecimalField(max_digits=12, decimal_places=2, default=0, verbose_name='الإجمالي النهائي')
    total_text = models.CharField(max_length=500, blank=True, verbose_name='الإجمالي كتابةً')
    user = models.CharField(max_length=100, verbose_name='المستخدم')
    invoice_date = models.DateField(verbose_name='تاريخ الفاتورة')
    hijri_date = models.CharField(max_length=80, blank=True, verbose_name='التاريخ الهجري')
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    @classmethod
    def next_number(cls):
        """إرجاع رقم الفاتورة التالي بصيغة 4 أرقام"""
        last = cls.objects.order_by('-id').first()
        if last:
            try:
                return str(int(last.invoice_number) + 1).zfill(4)
